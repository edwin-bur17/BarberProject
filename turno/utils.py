...
def is_slot_available(barber, date, time_obj, duration_minutes=30):
    ...
    reservations = Reservation.objects.filter(barber=barber, date=date).exclude(status='X')
...
def available_slots_for_barber(barber, date, start_time, end_time, interval_minutes=30, duration_minutes=30):
    slots = []
